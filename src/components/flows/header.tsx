    ...
    <div className="flex flex-col gap-1.5 px-6 pt-5">
      <div className="flex flex-wrap items-center gap-3">
        ...
        <button
          ...
          title={uiText("Back to Flows")}
          aria-label={uiText("Back to Flows")}
        ...
        <input
          ...
          placeholder={uiText("Flow name")}
          spellCheck={false}
          aria-label={uiText("Flow name")}
        ...
        {dirty && (
          <span
            ...
            title={uiText("Unsaved changes — hit Save to persist")}
            aria-live="polite"
          >
            <span className="h-1.5 w-1.5 rounded-full bg-amber-400" />{uiText("Edited")}</span>
        )}
        ...
        <div className="ml-auto flex flex-wrap items-center gap-1.5">
          ...
          >
            <History className="h-3.5 w-3.5" />{uiText("Runs")}<span className="ml-0.5 rounded bg-muted px-1.5 py-0.5 font-mono text-[11px] text-muted-foreground">
          ...
          >
            <Trash2 className="h-3.5 w-3.5" />{uiText("Delete")}</Button>
          {state.status === "active" ? (
            <Button
              variant="outline"
              size="sm"
            ...
            <Button
              ...
              title={
                !canActivate
                  ? uiText("Fix the issues below before activating")
                  : undefined
              }
            >
              {activating ? (
      ...
      <input
        ...
        placeholder={uiText("Add a short description (internal — customers don't see this)")}
        aria-label={uiText("Flow description")}
